 ...
 */
function bruteForceCanConstructWord(targetWord, wordBank) {
  if (targetWord === '') return true
  for (let word of wordBank) {
    if (targetWord.indexOf(word) === 0) {
      const suffix = targetWord.substring(word.length)
      if (bruteForceCanConstructWord(suffix, wordBank) === true) {
        return true
      }
    }
  }
...
module.exports.bruteForceCanConstructWord = bruteForceCanConstructWord;
